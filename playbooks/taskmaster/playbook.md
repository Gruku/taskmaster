...
**Legend:** where a target has already been converted to playbook form, the
"Route To" column shows the playbook path first, with the native invocation
as a parenthetical hint (e.g. `../start-session/playbook.md` *(taskmaster:start-session)*).
Targets not yet converted route via their native skill name only.

## Intent Detection

| Intent Signal | Route To |
|---|---|
| "Let's get started", "orient me", "show the backlog" | `../start-session/playbook.md` *(taskmaster:start-session)* |
| "Pick task X", names a task ID, "what should I work on" | `../pick-task/playbook.md` *(taskmaster:pick-task)* |
| Implementation request, in-progress task exists | Work in current task's worktree |
| Implementation request, no in-progress task | `../pick-task/playbook.md` *(taskmaster:pick-task)* first |
...
| "End session", "wrap up", "mark task done" | `../end-session/playbook.md` *(taskmaster:end-session)* |
| "Set up taskmaster", "initialize backlog" | `taskmaster:init-taskmaster` |
| "Write a handover", "for tomorrow" | `../handover/playbook.md` *(taskmaster:handover)* |
| "Log an issue", "file an issue" | Word-agnostic intake — `taskmaster:issue` if evidence cited, else `taskmaster:bug` |
