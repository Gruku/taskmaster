...
**A repaired file and a store holding an unwritten edit no longer drift apart for good (B-089).** Shipped since 6.0.0. When the store commits an edit to an entity whose projection file is quarantined — git conflict markers, damaged frontmatter — the export is suppressed, and `_replace_projection` recorded no `projection_base` for it. Repairing the file afterwards then read as an external edit with nothing to merge against, and `_merge_dirty_external_edit` returned without a word. From then on the store kept a committed edit it would never write, the file kept content the store would never import, and nothing told anyone. Any project that has ever had a file quarantined may be carrying such a pair now. The silent return was not limited to quarantine: any dirty row with no base took it. A missing or unparseable base now merges against an empty one, which sends the disagreement through the same local-wins rule as every other external edit and records it as a merge, so it counts in `Merge conflicts (24 h)`. A repair that agrees with the store clears the quarantine as well, instead of leaving a healthy file marked broken with its export still stranded. With no base to tell which side changed what, local-wins means the store's value is kept for every field the two sides disagree on, and the store's body is kept whenever the bodies differ; only fields present in the repaired file and absent from the store are taken from the file. Check a repaired file against what the store wrote back if the repair carried changes of its own.
...
**No schema version bump.** Nothing in this release adds a table or a column, so 6.0.2 and 6.0.3 processes can run against the same project.
