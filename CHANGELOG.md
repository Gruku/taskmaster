...
Every kind — task, epic, phase, bug, issue, handover, decision, idea, note, area, tracker and the project row — reads and writes through the store, and ids for every one of them are allocated inside the writing transaction from authoritative state (live rows, archived rows, reserved tombstones and the directory scan). An id is never handed out twice, including from a linked worktree whose files lag the store, so two agents creating a bug or a handover at the same moment no longer collide. `taskmaster_v3.py` keeps only pure parse, render, validate and path helpers; every writer it used to own is gone, along with the compatibility allowlist that wrote its own markdown.
...
**New: `backlog_store_status`**, the store health report — root and how it was resolved, schema version and creation token, database and WAL size, max seq, dirty and quarantined projection files, live sessions holding the writer, merge conflicts in the last 24 hours, the pending Linear push count, and the last 20 changes. It is genuinely read-only: it will not create a store that does not exist yet and will not move a damaged one aside, because a diagnostic that repairs the evidence is worse than one that says nothing.
...
**Hooks read the store, never build it.** All three merge and resurfacing hooks resolve the root through the shared rule in `taskmaster/root.py` — `TASKMASTER_ROOT`, else the git common directory, else the nearest ancestor holding a `.taskmaster/` — which is what makes a hook that fires inside a linked worktree act on the main checkout's backlog. `edit_resurface` and `merge_gate_decide` open `store.db` read-only when it exists and fall back to reading files otherwise; `merge_gate_decide` takes its policy from the project row. `merge_recorder_stamp` pins the resolved root and delegates its write to `backlog_record_merge`, so it goes through the same store transaction as any tool. No hook triggers an import or a rebuild, and the SessionStart index warm-up is gone.
...
**Migrations and scripts.** `backlog_migrate_v3` and `backlog_migrate_v4` (one adoption path, two historical names) canonicalize the layout if needed and then open the store, which performs the adoption; they no longer write files themselves, and adoption assigns deterministic ids to id-less epics, phases and tasks, logging each. `scripts/backfill_tldr.py`, `scripts/migrate_links.py` and `scripts/migrate_handover_statuses.py` write through the store in a single transaction, and `--dry-run` opens no write transaction.
...
- **`local/index.db` and `taskmaster/index.py` are gone.** The 5.2.0 derived index is absorbed into `store.db`; the path helpers that survived (`normalize_task_anchor`, `normalize_location`, `extract_prose_paths`, `infer_repo`, `REVERSE_TYPE`) moved to `taskmaster/paths.py`.
