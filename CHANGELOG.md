...
---
## Unreleased — SQLite store, steps 1 to 3
...
Defects 1–5 from that spec are fixed **for every entity kind**. Ids for tasks, epics, phases, bugs, issues, handovers, decisions, ideas, notes, areas and trackers are allocated inside the writing transaction from authoritative state — live rows, archived rows, reserved tombstones and the directory scan — so an id is never handed out twice, including from a linked worktree whose files lag the store. Two agents creating a bug or a handover at the same moment no longer collide.

Every entity writer now commits through the store; the compatibility allowlist that used to write its own markdown is gone, along with `taskmaster_v3`'s writers. Step 3 also adds `backlog_store_status`, a read-only health report that never creates or repairs a database; a ` [seq N]` suffix on every mutating tool's result, naming the `changes` row that commit produced, with an `(export pending: …)` notice when the commit landed but a file the caller can see did not; `ideas/IDEAS.md` as derived output regenerated from the idea rows rather than a second writer of one file; and the Linear queue as a `linear_queue` table, with any legacy `integrations/linear-queue.json` imported once on first open and then removed.

Two boundaries remain. `backlog_migrate_v3` and `backlog_migrate_v4` still write through `save_v3`/`save_v4`, behind the store rather than through it; the next transaction's import scan re-adopts what they wrote. The viewer's read endpoints and the derived index in `index.py` land in step 4.
...
Every mutating tool now renders its response from the committing transaction's captured result, not from the in-flight dict, so a tool can only report what the store actually kept. Nested calls no longer overwrite the outermost tool's response: the renderers are a per-call stack.
