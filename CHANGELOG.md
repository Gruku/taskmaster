...
**A repaired file and a store holding an unwritten edit no longer drift apart, and the repair is not overwritten (B-089).** Shipped since 6.0.0. When the store commits an edit to an entity whose projection file is quarantined (git conflict markers, damaged frontmatter), the export is suppressed, and no merge base was recorded for the file. When the file was repaired later, `_merge_dirty_external_edit` found nothing to merge against and returned without a word. The store then held a committed edit it would never write, and the file held content the store would never import. Nothing told anyone. Any project that has ever had a file quarantined may be carrying such a pair now. A repaired file is now merged three ways against the state the file and the store last agreed on:

- A field only the repair changed, or removed, keeps the repair's value.
- A field only the store changed keeps the store's value.
- Only a field both sides changed to different values is a conflict. The store's value is kept, and the file's value is recorded in that merge's `changes` row.

The base comes from one of two places. Quarantining a file now records the entity as it stood, while it still matched the file. A file quarantined by 6.0.2 or earlier has no recorded base, so the store rebuilds it by walking the entity back through every change the file never received: those after its last export, or after its last import. If any of those changes can't be reversed exactly, there is no base. The store doesn't guess one. In that case nothing is merged. The repaired file is kept as written, and the store's version of every field that differs is kept in full in the merge's `changes` row. Every merge conflict, including this no-base case, now appears in the next tool result: as a `Warning: merge conflict in <file> …` line on a text result, or as a `merge_conflicts` list on a JSON object. It names the fields and the `changes` seq that holds the side that was set aside. It is also still counted in `Merge conflicts (24 h)`. A result that is a JSON array has no place for the warning, so the next result carries it instead. A repair that matches the store clears the quarantine instead of leaving a healthy file marked broken with its export stranded. **An earlier 6.0.3 candidate merged against an empty base instead.** That kept the store's value for every field the two sides disagreed on, and it reverted a user's own repair and wrote the store's version over their file. It never shipped.
...
**An export that cannot land is rendered and logged once, not once per write (B-085).** Every write to an entity whose file was quarantined rendered the whole document again and appended another identical `projection export suppressed` line to `local/store.log`: 20 writes, 20 renders, 20 lines, for as long as the file stayed broken. The second and later suppressions now stop at one indexed lookup, still warn the caller with `export pending: <file> is quarantined`, and render and log nothing; the lookup runs only when the transaction has any quarantined row, so an adoption exporting thousands of entities does not pay for it. `backlog_store_status` gains a `Stuck exports` line naming files the database holds newer content for and cannot write. **This is not faster.** On a small fixture the 20 writes took about 10 % longer (486 ms before, 536 ms after): the render it saves costs less than the transaction's `fsync`, and the lookup is visible. What it buys is a bounded `store.log` and the fix above.
...
**No schema version bump.** Nothing in this release adds a table or a column, so a 6.0.2 store opens in 6.0.3 unchanged. A 6.0.2 process should not keep running against it, though. See the upgrade note at the top of this entry.
