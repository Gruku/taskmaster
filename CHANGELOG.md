...
Called with no arguments, it lists flagged files. A file that is missing or does not parse cannot be taken, and the call says so without changing anything. The store never picks a side on its own.

A hand edit to a file whose last export failed to land (a file another program held open, not a quarantined one) is still merged three ways against the bytes that export left in place, as in 6.0.2. If those bytes are missing or don't parse, the file is flagged instead.
...
**No schema version bump.** This release adds one table, `projection_conflict`, which holds the flags described under B-089, and one index, on `projection(kind, id)`. Both are created with `IF NOT EXISTS` on open, and `SCHEMA_VERSION` stays at 1, so a 6.0.2 store opens in 6.0.3 without a rebuild. The first 6.0.3 open of such a store takes the writer mutex to create the table. A 6.0.2 process doesn't know about the table and would export over a flagged file, so this is one more reason not to leave 6.0.2 running against the project. See the upgrade note at the top of this entry.
