...
**Before upgrading, stop every Taskmaster 6.0.2 process on the machine: MCP servers, the viewer, and any open Claude Code session that loaded the plugin.** A 6.0.2 process still has the B-092 defect described below. The first commit a 6.0.3 process makes to a shared project can make a running 6.0.2 process rename that healthy `store.db` aside and rebuild it from the files. Everything that lives only in the database is lost from the live store when that happens. 6.0.3 can't protect a store from a 6.0.2 process, so the only safe upgrade is to stop all 6.0.2 sessions first, then start 6.0.3.

**A write on a large backlog no longer takes seconds, and three agents writing at once no longer queue behind each other for half a minute.** Every write in 6.0.x rebuilt the `related` table from scratch while holding the writer lock: it compared every anchor and location row in the project with every other one. On a backlog with 3,857 such rows (CodeMaestro, about 3,700 entities) that is 7.4 million comparisons and 2.2 million glob matches per write, 62-75 % of the write's time, and it grows with the square of the row count. A write now re-pairs only the rows of the entities it touched and removes their old edges from both sides; a write touching half the rows or more, such as `backlog_index_status(rebuild=True)` or first adoption, still rebuilds from scratch. The table's contents are unchanged: 600 seeded edit sequences and 24 seeded sequences of real transactions compare it with the 6.0.2 full rebuild row for row, weights included. Measured on a copy of the CodeMaestro backlog, a fresh copy per run, one run at a time, with no test or parity process running before or after any run (AMD Ryzen AI 9 HX 370, 31 GB, Windows 11; 6.0.2 as installed, 6.0.3 at `4bc34ad`), median of 4 writes: task field update 7.06 s to 2.12 s, status change 8.82 s to 1.93 s, note create 8.71 s to 1.74 s, handover create 8.72 s to 2.55 s. The `related` refresh went from 4.8-8.3 s per write to 0.03-0.14 s. Three processes making 4 task updates each: median 13.8 s to 4.1 s, slowest 32.0 s to 9.2 s, no `store busy` errors in either run (6.0.2 has been seen failing with `store busy for 30s` at this load). The flagged-file checks this release adds to every write (B-089) did not show up: the rest of the write, everything except the `related` refresh, was 0.25-0.9 s faster in 6.0.3 than in 6.0.2 (warm writes), not slower, at a resolution of about 0.3 s. A write still takes 1.6-2.7 s on this backlog, 3-4 s for the first write in a process; most of what remains is the projection scan and loading the backlog.

**Two processes on one project no longer rename a healthy `store.db` aside as corrupt (B-092).** Released 6.0.x could destroy a working database whenever a second Taskmaster process committed to it — two MCP servers, a session plus a hook, a viewer plus a tool call. `PRAGMA quick_check` on a connection that has already inspected the FTS5 index reports `malformed inverted index for FTS5 table main.entity_fts` as soon as another connection commits to that index. That verdict describes state held inside the connection, not the file: every fresh read-only snapshot of the same database reports `ok`. `Store._prepare_schema` raised on it anyway, `"malformed"` is a corruption marker, and `_ensure_open_locked` sent the open to `_recover_corrupt_database`, which renames `store.db`, `-wal` and `-shm` to `store.db.corrupt-<timestamp>` and rebuilds from the file projection. The projection is a lagging export, so the rebuild kept the backlog and lost everything that lives only in the database: recorded progress entries, live session rows, Linear queue claims and the quarantine log. **On macOS and Linux the rename succeeds on a file other processes hold open, so this completed silently.** On Windows the rename failed with `WinError 32` and the copy fallback with `WinError 33`, so the call errored out loudly and the store survived — by accident. Recovery now acts on a retained connection's verdict only after `Store._corruption_is_real` confirms it: a fresh read-only connection runs a full `PRAGMA integrity_check` in one snapshot, and only a non-`ok` result there, or a file a fresh connection cannot parse at all, is treated as damage. Both outcomes are written to `local/store.log`. An unconfirmed verdict is not acted on: a failed `quick_check` lets the open continue, and a corruption error caught while opening drops this thread's connections and retries schema preparation once on a new one. Nothing is renamed, copied or rebuilt. A snapshot that cannot be opened (missing, locked) is not confirmation. Real damage is still found and recovered as before, the byte-level header check is unchanged, and the check that confirms is stricter than the one that triggers it. Two limits: the confirmation needs SQLite 3.44 or newer, the first release whose `integrity_check` checks FTS5 at all, so a Python linked against an older SQLite keeps the old behaviour; and a genuine non-`ok` verdict now costs a full `integrity_check`, seconds on a large store rather than the ~200 ms cold-open probe. Measured on Windows against the two-process concurrency test that exposed it: 4 failures in 60 isolated runs before, 0 in 60 after. Windows recovery still cannot rename a database another process holds open; that is left as it was on purpose, because fixing it first would have made this loss reachable on Windows too.

**A file repaired while the store holds an unwritten edit to it is flagged, and both versions are kept (B-089).** Shipped since 6.0.0. When the store commits an edit to an entity whose projection file is quarantined (git conflict markers, damaged frontmatter), the export is suppressed. When the file was repaired later, `_merge_dirty_external_edit` found nothing to merge against and returned without a word, and a repaired `backlog.yaml` did the same. The store then held a committed edit it would never write, and the file held content the store would never import. Nothing told anyone. Any project that has ever had a file quarantined may be carrying such a pair now. The first scan under 6.0.3 flags it.

Nothing is merged any more. When a quarantined file parses again while the store holds an edit it has not written there:

- The store keeps its entity as it is, and the file stays on disk exactly as you wrote it.
- The file is **flagged**. While it is flagged, no export writes, moves or deletes it: not a later edit to that entity, not an archive, not a delete. The bytes the store last read from the file are kept with the flag.
- Every tool result names the flagged file until it is resolved, as a `Warning:` line on a text result or a `projection_conflicts` list on a JSON object. A result that is a JSON array has no place for it. `backlog_store_status` lists flagged files on a new `Flagged` line.
- A repair that already agrees with the store, field for field, is not a conflict. The quarantine clears and the pending export lands.
- A repair with nothing pending is imported as an ordinary hand edit, as before.

This covers task, epic, phase and every other entity file, `project.yaml`, and `backlog.yaml`. Editing a flagged file again keeps it flagged and unimported, and the kept bytes follow your latest edit. Editing it until it matches the store clears the flag.

**What you'll see:** `Warning: tasks/T-12.md (task T-12) was edited while the store held a change it had not written there yet, so neither version was applied to the other. Both are kept: the store has its version, the file is untouched, and exports to it are paused. Compare them with backlog_resolve_conflict(file="tasks/T-12.md"), then keep one with take="file" or take="store".`

**To resolve it,** call `backlog_resolve_conflict(file=…)` to see the file as it is on disk, the file as the store last read it if that differs, and the exact text the store would write. Then call it again with one of these:

- `take="file"` imports the file as it is on disk now. The store values it replaces stay in that import's `changes` row.
- `take="store"` writes the store's version over the file, or moves or removes the file when the entity was archived or deleted. The file text it replaces is kept in the resolution's `changes` row (`op='resolve'`).

Called with no arguments, it lists flagged files. A file that is missing or does not parse cannot be taken, and the call says so without changing anything. **A resolution always takes a whole file.** For `backlog.yaml`, that means every epic and phase entry in it at once. There is no per-entity choice, and the tool's answer says so. The store never picks a side on its own.

A flag keeps one copy of the file's bytes in the store: the last version a scan read. The copy is replaced only when the file's contents change, and deleted when the flag is resolved. Rescanning an unchanged flagged file writes nothing. A flagged `backlog.yaml` therefore costs one extra copy of that file, not one per scan.

A hand edit to a file whose last export failed to land (a file another program held open, not a quarantined one) is still merged three ways, as in 6.0.2, but only against bytes the store actually saw on disk: the base must hash to the content the projection row last recorded. A base that doesn't qualify is not merged against, and the file is flagged instead. That covers a missing base, one that doesn't parse, a rendered entity left by an unreleased 6.0.3 build, and an older base 6.0.2 kept after a merge moved the row on.

**Two earlier 6.0.3 candidates merged automatically, and neither shipped.** One merged against an empty base, so the store won every disagreement and reverted the user's repair. The other rebuilt a base from the change history. Review reproduced silent data loss with it: a repair undid a store edit, a user's own revert was undone, stale fields in an epic file reverted a `backlog.yaml` rename, and `backlog.yaml` still stayed stuck. A merge needs the state both sides last agreed on, and the store cannot reliably reconstruct it.

**Archiving or deleting an entity no longer deletes its quarantined file.** Shipped since 6.0.0. Archiving a task whose file was quarantined wrote the archive copy and deleted the broken live file, along with anything written in it that the store had never been able to import. While any file of an entity is quarantined or flagged, no export writes, moves or removes any of that entity's files. The store records the pending export and warns `export pending: <file> is quarantined`. Once the file is repaired, the archive shows up as a flagged difference for you to resolve.

**Databases renamed aside as corrupt are no longer deleted.** Opening a store used to delete every `store.db.corrupt-*` file whose modification time was over 7 days old. A rename keeps the original modification time, and in WAL mode `store.db`'s modification time changes only on a checkpoint. A database renamed aside today could therefore be deleted on the next open. Nothing deletes these files now. They sit next to the live database, in `.taskmaster/local/` of the main checkout (a linked worktree uses the main checkout's store): `store.db.corrupt-<UTC timestamp>` with its `-wal` and `-shm` files. `backlog_store_status` lists them on its `Corrupt` line. **If a 6.0.x store was ever renamed aside, including by the B-092 defect below, that family may be the only copy of the progress entries, session rows, Linear queue claims and quarantine log committed before the rename.** Don't delete it until you've checked what it holds. Open a copy of all three files together with `sqlite3`, not the originals, and never copy one over `store.db` while any Taskmaster process is running. Once you no longer need a family, deleting it is your call.

**A write that committed is no longer reported as failed.** After the commit, `transaction()` and `rebuild_derived()` refresh the session row, and that refresh now checks store compatibility. If a migration fence was published in that moment, the refusal escaped as the call's error. A caller told the write failed would retry it and create a duplicate. The refusal is now ignored after the commit. Nothing in 6.0.3 publishes a fence, so no release reached this.

**A forked child no longer inherits a held writer gate.** On macOS and Linux, a host process that forked while one of its threads was waiting for or holding the store's in-process writer queue left the child hanging or timing out on every write. The child now starts with a fresh queue.

**An export that cannot land is rendered and logged once, not once per write (B-085).** Every write to an entity whose file was quarantined rendered the whole document again and appended another identical `projection export suppressed` line to `local/store.log`: 20 writes, 20 renders, 20 lines, for as long as the file stayed broken. Every suppression now stops at one indexed lookup before anything is rendered, still warns the caller with `export pending: <file> is quarantined`, and logs only the first time; the lookup runs only when the store has any quarantined or flagged row, so an adoption exporting thousands of entities does not pay for it. `backlog_store_status` gains a `Stuck exports` line naming files the database holds newer content for and cannot write. **This is not faster.** On a small fixture the 20 writes took about 10 % longer (486 ms before, 536 ms after): the render it saves costs less than the transaction's `fsync`, and the lookup is visible. What it buys is a bounded `store.log` and the fix above.

**A writer waiting on the store says so, gives up when told, and waits its turn (B-083, B-084).** `update_root_config`, `linear_mark`, `linear_requeue`, `linear_claim` and `rebuild_derived` took the writer mutex with no timeout, so they waited the full 30 s in silence, and the busy error then reported 30 s whatever the caller had asked for. All five take `timeout_ms`, and the error names the wait actually served and the operation: `store busy for 0.4s while update_root_config waited; probable holders: …`. A waiting MCP tool writes a notice to stderr, where the host logs it, at 0.5 s and every 2 s after; the first names the probable holders. Notices go through a bounded queue drained by a background thread and are dropped when it is full — written inline, a host that stopped reading stderr stalled a queued writer past its 30 s deadline to 82 s. Within one process, waiting threads now get the lock in arrival order; before, eight threads arriving 50 ms apart were served `[3, 4, 0, 5, 1, 6, …]`. Across processes a file lock gives no queue, so order is still not guaranteed, but the poll is jittered and shortens the longer a caller has waited.

**A read that finds the store busy retries its scan instead of skipping it (B-086).** The read-side scan set its 2 s throttle before it knew whether it would run, so a scan skipped because another process held the writer still used up the window. On a store busy whenever a read landed, hand edits were never adopted, and that looked exactly like "nothing changed". The throttle now moves only when a scan completes; a skipped scan makes the next read retry the lock straight away without re-walking the projection. Consecutive skips warn every fifth time and appear in `backlog_store_status` as `Read-scan skips: N  (store busy: hand edits are not being adopted)`. The count is per process: a status report from a process that never opened the store reads 0.

**Reads copy and decode less (B-087).** A read that published to the cache deep-copied the whole backlog twice, every read JSON-decoded every non-task entity in the project whether or not it asked for them, and a single-entity read built the whole backlog to reach one row. The cache now keeps the object the call already owns and the caller gets one copy, made by a plain copier for JSON-shaped data; non-task kinds are decoded when first used, from rows still fetched inside the caller's snapshot; and `_store_read_entity` reads one row through the new `Store.entity_row`. On a 2,050-task fixture a warm `load_dict()` went from 21.4 ms to 9.1 ms and a single bug read from 8.0 ms to 0.37 ms; the fixture's documents are smaller than a real backlog's, so the ratios carry over better than the milliseconds. `backlog_get_task` still builds the whole backlog and benefits only from the cheaper copy.

**A store from a newer Taskmaster is refused, not rebuilt.** A database whose `schema_version` is higher than this client understands used to take the rebuild branch: every table dropped and rebuilt from the projection, losing the same database-only state as above. The server, the viewer, scripts, `checkpoint_all` and both read-only hooks (`edit_resurface`, `merge_gate_decide`) now check `meta` before configuring the journal, registering a session, preparing the schema or attempting recovery, and refuse with `Unsupported Taskmaster schema_version=…; this client supports 1. Upgrade the client; the database has not been rebuilt.` A `minimum_client_protocol` above 1, or a `migration_state` other than `ready`, is refused the same way; writers check again after taking the write lock. A 6.0.3 store records `bridge_client_protocol` and `bridge_capabilities` in `meta` for future migrations to read. A database missing those markers — every one written by 6.0.2 — is admitted unchanged, and a 6.0.2 process ignores them. **This reverses 6.0.2's note on pre-release stores:** a database that an unreleased 6.0.2 build stamped `schema_version=2` is no longer rebuilt on open; it is refused until that value is set back to 1 by hand.

**No schema version bump, but one new table.** This release adds a table, `projection_conflict`, which holds the flags described under B-089, and an index on `projection(kind, id)`. Both are created with `IF NOT EXISTS` when the store opens. `SCHEMA_VERSION` stays at 1, following 6.0.2's additive columns, so a 6.0.2 store opens in 6.0.3 without a rebuild. The first 6.0.3 open takes the writer mutex to create the table. **Flags are enforced only once no 6.0.2 process is running against the project.** A 6.0.2 process ignores `projection_conflict` entirely: it will export over a flagged file, and it will still archive a quarantined file away. Stop every 6.0.2 process first, as the upgrade note at the top of this entry says.

---
## 6.0.2

**A warm read on a large backlog no longer hangs (B-082).** On a 2,050-task project every read tool ran until the harness gave up: `backlog_list_tasks`, `backlog_status`, `backlog_search` and `backlog_get_task` all timed out, so nobody could list tasks by status. Defects on the read path compounded, in six places:

- **A permanently quarantined file forced a full scan on every read.** `Store._projection_changed_on_disk` returned "changed" whenever any projection row was quarantined, and an unrepairable file is re-quarantined by every scan, so the row never cleared. The `projection` table now carries `quarantine_mtime`, `quarantine_size` and `quarantine_hash`, taken from the bytes that were parsed rather than from a fresh stat afterwards — stat-ing the path again described whatever was on disk by then, so an editor saving a repair between the parse and the stamp made the store record the *good* file and skip it for good. The stamp is written when the reason reaches `local/store.log`, and a quarantined row whose file still matches it is treated as unchanged by both the change check and the scan instead of being re-read and re-parsed twice per scan. When `force_hash` is on — a branch switch — the stamp's digest is compared against the bytes on disk instead of the stat, because that is exactly the case where a checkout can restore a file at its old size inside one mtime tick. A quarantine whose log append failed leaves the stamp unset and is retried, so a reason is never silenced for good. An unparseable `project.yaml` never gets a projection row at all: it settles through the reason recorded in the `quarantine_log` meta row, which a read-side probe now commits rather than rolling back (see below). That record now carries the size and the digest of the bytes it was written for as well as their mtime — on the mtime alone, a timestamp-preserving restore of different bytes, or a repair inside one tick, stayed invisible for the life of the project. A two-element signature written by 6.0.1 proves nothing and is rescanned once, which rewrites it in full.
- **The git index made every git command re-hash the whole backlog.** `Store._git_generation` read and SHA-1'd all of `.git/index`, which on a monorepo is megabytes and is rewritten by every git command — so `force_hash` was usually on and every projection file was read and hashed before the scan, then again inside it. The index is no longer part of the token in any form, not even by its stat: `git status` rewrites it to refresh its stat cache, so with ten sessions on one repo the token moved every few seconds and every store re-hashed all 2,050 files (88 ms) on its next read, with `last_scan_generation` churning along with it. The token is now the content of `HEAD` and `ORIG_HEAD`, so a branch switch still forces the hash comparison; what it gives up is a same-size restore inside a single mtime tick on a coarse-timestamp filesystem, which a non-git restore (`tar`, `rsync -t`) would miss anyway. The generation is computed once per read and passed into the scan, and a sweep that proves a generation clean is remembered for the process together with `PRAGMA data_version` — the revision another connection's commit moves — so another process importing under a second generation retires the memo instead of leaving this reader trusting a sweep that no longer describes the tree. **Every store pays one extra full sweep the first time it runs 6.0.2**, because the token it has stored was computed the old way.
- **A cold process took the 30 s writer mutex just to open an existing database.** A fresh MCP server doing a plain read queued behind any writer and then failed with nothing to show. A database that exists, passes `quick_check`, and is already at the current schema is now adopted without the mutex; the mutex still guards schema creation, upgrades, corruption recovery and the first bootstrap. The unlocked path writes nothing at all: a database whose `local/id-reservations.json` is missing falls through to the locked path rather than rebuilding the file with an unserialized read/merge/replace that could drop a reservation another process had just made — and hand the id it protected out twice.
- **A read that found nothing new still committed.** `Store._maybe_scan_on_read` opened a transaction with an empty body and ran the whole write pipeline — derived tables, exports, PROGRESS.md, checkpoint — every time. A read-side probe whose scan changed nothing now rolls back instead. Ten server processes were otherwise committing several times a second on an idle backlog. A probe that recorded something counts as changed even when it wrote no row: a quarantine reason and a pending `store.log` entry are already on disk when the probe decides, so rolling them back re-logged the same line on every later read and, for a file with no projection row, left the change check unsettled for good.
- **The entity inventory ran eighteen globs over eleven kinds, twice per read.** `Store._known_entity_files` now does one `os.scandir` per directory, memoized across a read's change check and the scan it triggers, with the same canonical-first ordering and the same skips for dotfiles, `.tmp.` and `.corrupt-`. Matches are ordered by `os.path.normcase` of the whole path, which is the key `sorted(Path.glob(...))` compared by — sorting bare names instead put `handovers/_archive/Z/` ahead of `handovers/_archive/a/`, so a duplicate id across two archive directories resolved to a different file than before.
...
**No schema version bump.** `quarantine_mtime`, `quarantine_size` and `quarantine_hash` are added additively on every open, like the Linear queue's claim columns before them, and `SCHEMA_VERSION` stays at 1 — so a 6.0.1 process still running against the same project keeps working instead of reading a higher version as an incompatible schema, dropping every table and rebuilding it without the new columns. The unlocked cold open asks whether the columns are present rather than what the version says; a database written before them falls through to the mutex path, which adds them.

**If you ran this branch before it was released, your store rebuilds itself once.** Two commits on the way to 6.0.2 carried `SCHEMA_VERSION = 2` before the columns were made additive, and a database opened by them wrote `schema_version=2` into `meta`. The released code is back at 1, reads 2 as a version from the future, and takes the rebuild branch: every table is dropped and rebuilt from the projection on the first open. The backlog itself survives — it is the projection — but state that lives only in the database does not: recorded progress entries, live session rows, Linear queue claims and the quarantine log all start empty. Nothing shipped ever wrote a 2, so this reaches only a machine that ran the branch directly.

**The per-read directory listing is per-thread.** `Store._memoized_entity_files` saved and restored the memo on one shared attribute, but connections are per-thread and reads run concurrently — an interleaving could restore one thread's listing after it had already left, freezing the inventory for the life of the process so new files were never seen.
...
**A permanently quarantined file is logged once, not on every read.** A file the store cannot parse — git conflict markers, frontmatter damaged years ago — opts out of the scan's stat/hash shortcut, so every warm tool call re-read it, re-quarantined it and appended the same reason to `local/store.log`. Four such files grew the log without bound. `Store._note_quarantine` now warns the caller every time but writes the log only when the reason or the file's mtime is new, keyed in the `meta` row `quarantine_log`; `Store._prune_quarantine_log` forgets a file once it is repaired or removed, so a relapse is recorded again. The line is appended before the signature is recorded and `Store._log` reports whether it landed, so an unwritable `store.log` — or a crash before the append — leaves the reason still owed rather than silenced for good; `Transaction.flush_quarantine_log` writes the state once per transaction, from `_capture_committed`, so it still rolls back with everything else. An unparseable `project.yaml`, which never gets a projection row at all, took the same path and is covered too.

**A file created or archive-moved on a CRLF project is written CRLF.** In-place rewrites already preserved each file's own line endings, but a path with no bytes to probe fell back to LF, so adoption of a Windows backlog left `tasks/` carrying both styles. `_probe_crlf` now separates "this file is LF" from "there is nothing to match", `Store._dominant_line_ending_crlf` samples the project's existing entity files once per store to decide what a new file gets — up to twenty per directory, each directory's vote weighted by how many files it actually holds, so two thousand tasks are not outvoted by forty files in two small directories — `Store._export_entity_row` reads the old path's style before the archive move deletes it, so a moved file keeps the endings it had. A project with nothing to sample still gets LF.
...
Every kind — task, epic, phase, bug, issue, handover, decision, idea, note, area, tracker and the project row — reads and writes through the store, and ids for every one of them are allocated inside the writing transaction from authoritative state (live rows, archived rows, reserved tombstones and the directory scan). An id is never handed out twice, including from a linked worktree whose files lag the store, so two agents creating a bug or a handover at the same moment no longer collide. `taskmaster_v3.py` keeps only pure parse, render, validate and path helpers; every entity writer it used to own is gone, along with the compatibility allowlist that wrote its own markdown. The one direct file write left in the module serves `local/viewer.json`, machine-local viewer preferences rather than backlog data.

**Every mutation proves itself.** A mutating tool's string result ends in `[seq N]`, naming the row in the `changes` table that the commit produced; a JSON-returning tool carries the same number in a `seq` key, and so does a successful viewer POST/PATCH/PUT/DELETE. When the row committed but a file the caller can see did not, the result also carries `(export pending: <file> — retried on next call)`: the write landed and only the export is being retried. Responses are rendered from the committing transaction's captured result, never from the in-flight dict, and nested calls no longer clobber the outermost tool's response because the renderers are a per-call stack.

**New: `backlog_store_status`**, the store health report — root and how it was resolved, schema version and creation token, database and WAL size, max seq, dirty and quarantined projection files, live sessions, merge conflicts in the last 24 hours, the pending Linear push count, and the last 20 changes. It is genuinely read-only: it will not create a store that does not exist yet and will not move a damaged one aside, because a diagnostic that repairs the evidence is worse than one that says nothing.

**`backlog_query` and `backlog_search` run against the store.** `backlog_query` executes guarded read-only SQL over `store.db` — tables `entities`, `changes`, `projection`, `sessions`, `linear_queue`, `links`, `related`, `entity_paths`, `handover_tasks` and `entity_fts` — and sees a hand edit after the read throttle; `backlog_search` ranks through the store's FTS. `backlog_index_status` now reports derived-table row counts and the active YAML loader in three lines (`Store:` / `Rebuilt:` / `Rows:`); with `rebuild=True` it recomputes the derived tables only, never the authoritative ones, and unlinks a leftover `local/index.db`.

**Hooks read the store, never build it.** All three merge and resurfacing hooks resolve the root through the shared rule in `taskmaster/root.py` — `TASKMASTER_ROOT`, else the git common directory, else the nearest ancestor holding a `.taskmaster/` — which is what makes a hook that fires inside a linked worktree act on the main checkout's backlog. `edit_resurface` and `merge_gate_decide` open `store.db` in query-only mode — `mode=rw` plus `PRAGMA query_only=ON`, so the connection can never write and can never bring a missing database into being — and neither one imports or rebuilds anything. With no store on disk they diverge on purpose: `merge_gate_decide` reads the projection instead, because a gate is worth most on a machine where the backlog is still legible on disk, while `edit_resurface` logs `no store at <path>; staying quiet` and exits, because an ambient one-line notice is not worth seconds on every edit. A store that exists but cannot be read is a different case, and the gate now **fails open**: it logs `store unreadable (…); allowing the merge` and allows. Once a store exists it is the only authority, the files are a lagging export of it, and reading them could block a merge the store cleared seconds ago from a gate that has not reached `tasks/<id>.md` yet. `merge_gate_decide` takes its policy from the project row.

`merge_recorder_stamp` is the one hook that writes, and it no longer bootstraps anything. It looks the task up by branch on a `mode=rw`, `query_only` connection that refuses to create a database, so a merge in a project with no store is logged as `no store at <path>; not recording this merge` and simply not stamped — going through the server's loader for that lookup is what once let a PostToolUse hook build a whole project. When a store is there, the stamp goes through `backlog_record_merge` like any tool, waiting up to 30 s for the writer rather than the merge gate's short timeout, because a merge fires once and a stamp lost to a busy writer is lost for good. The SessionStart index warm-up is gone.

**Viewer.** Every GET serves committed rows under one ETag, `creation_token:max_seq` (a projection-only store on network storage answers `projection-<sha>:0` instead of a constant that never moves). A refused legacy layout is answered as a 409 with a body rather than a bare traceback. PATCH and PUT apply the full status-transition table and the completion gate, returning 409 with the reason; `/api/tasks/validate` runs the same check. An explicitly supplied `status` must be a real status: `{"status": null}` used to read as "this write names no status", skip the transition table entirely and persist a null the MCP tools reject and the board cannot place in any column. It is now rejected by name. An absent `status` key still means the write names no status and is left alone. `If-Match` is evaluated inside the write transaction. The continuity rail behind `/api/continuity` is built from store rows out of one loaded tree rather than by re-globbing the artifact directories the store already owns.

**Linear.** The retry queue lives in the `linear_queue` table (states `pending`/`claimed`/`done`/`failed`, with an attempts count). `linear.yaml` edits now go through the store's `update_root_config`, which holds the cross-process writer lock across the whole read-modify-write, so two agents adding a workspace at the same time no longer both report success while one addition is lost. A legacy `integrations/linear-queue.json` is imported once on first open — permanent failures import as `failed` — and the file is removed after that commit. An explicit retry un-parks failed rows.

**PROGRESS.md** is rendered by the store into a marker-delimited region (`<!-- taskmaster:session-log -->`) from a bounded log of the last 200 applied entries. Anything you write outside that region is preserved.

**The exporter keeps a file's line endings.** A projection file already using CRLF is rewritten as CRLF; a file the store creates is written LF. Only the first newline is inspected, so this costs nothing on a large file. Without it every export on Windows rewrote whole files that had not changed in substance.

**Adoption verifies itself before it commits.** Every file adoption renders — each entity, `backlog.yaml` and `ideas/IDEAS.md` — is parsed back with the same reader the import scan uses, and compared against the document it was rendered from, before `os.replace` puts it on disk. If any file does not survive the round trip, adoption refuses and names the entity (`adoption refused: the epic 'auth' does not survive a round trip through epics/auth.md (fields ['name'] read back differently). Nothing was changed.`). Adoption rewrites every file in the project, and nothing in that path used to check the result was still readable; when it was not, the next cold open raised before any tool ran and the store that could still have answered was the only copy left. A refused adoption also removes the empty database that opening the project created, so the refusal cannot leave behind a valid-looking store that silently disables the merge gate.

**Adoption moves machine-local files rather than deleting them.** `viewer.json`, `auto/`, `snapshots/` and `PROGRESS.md` are relocated under `.taskmaster/local/`, where the v4 reader looks for them; leaving `PROGRESS.md` at its pre-v4 path stranded it, and the next session log created an empty one while the real changelog sat unreferenced beside it. Nothing here deletes: an earlier build removed `snapshots/` outright, which was defensible while only an operator could reach it but not now that the first read tool or viewer GET on a pre-v4 project triggers it — `snapshots/` is the one directory that could recover a project from a bad adoption. A name already taken at the destination is never overwritten either; the incoming file takes the first free `-2`, `-3` suffix and the move is reported on the result.

**Adoption names what the files left unnamed.** An id-less epic, phase or task in a hand-edited `backlog.yaml` is given a deterministic id derived from its name, and each assignment is logged. An epic that exists only as `epics/<id>.md`, which `backlog.yaml` never mentions, now keeps its own id and name: with no slim half to merge against, the file is the only copy, so it supplies everything the row lacks. Previously its identity was dropped, `backlog.yaml` gained a `- {}` entry, and the next cold open refused the projection.

**Migrations and scripts.** `backlog_migrate_v3` and `backlog_migrate_v4` (one adoption path, two historical names) canonicalize the layout if needed and then open the store, which performs the adoption; they no longer write projection content themselves — canonicalization moves files, adoption imports them — and adoption assigns deterministic ids to id-less epics, phases and tasks, logging each. `scripts/backfill_tldr.py`, `scripts/migrate_links.py` and `scripts/migrate_handover_statuses.py` write through the store in a single transaction, and `--dry-run` opens no write transaction.

### Breaking

- **A legacy `.claude/` or project-root backlog is refused** until `backlog_canonicalize_layout` runs. The store exists at exactly one location, `<project root>/.taskmaster/local/store.db`; opening a backlog anywhere else used to redirect silently and bootstrap an empty database beside the real one. Canonicalizing now also moves `epics/`, `phases/`, `bugs/`, `decisions/`, `ideas/`, `notes/` and `integrations/`, which earlier releases left behind, plus `project.yaml` and `linear.yaml`. Those two are only ever read from the canonical location, so left in a legacy layout they do not fall back — the project's policies and its Linear config simply vanish, and a rerun answers `already_canonical` and never recovers them. Canonicalization refuses to run on network storage.
- **The backlog is per repository, not per branch.** The root resolves to the git common directory, so every linked worktree of a checkout shares one store.
- **The derived `context:` block is no longer written into `backlog.yaml`.** Read the same snapshot from `backlog_status` or `backlog_query`.
- **`local/index.db` and `taskmaster/index.py` are gone.** The 5.2.0 derived index is absorbed into `store.db`; `normalize_task_anchor`, `normalize_location`, `extract_prose_paths` and `infer_repo` moved to the new `taskmaster/paths.py`, which also re-exports `REVERSE_TYPE` from its home in `taskmaster_v3.py`.
- **`integrations/linear-queue.json` is gone**, replaced by the `linear_queue` table after a one-time import.
- **`ideas/IDEAS.md` is derived output**, regenerated from the idea rows whenever an idea is written. Do not hand-edit it; edits are overwritten.
- **`scripts/migrate_handover_statuses.py` takes `--root <project root>`** instead of a positional path to `backlog.yaml`.
...
- `/api/threads` and `/api/continuity` carry no ETag.
- `scripts/migrate_links.py` drops `depends_on` unless `--keep-legacy` is passed (pre-existing behaviour).
