...
def open_decision(backlog):
    did, _ = entity_helpers.write_decision(
...
def test_resolve_sets_status_resolved_with_and_timestamp(backlog, open_decision):
    fm = entity_helpers.resolve_decision(backlog, open_decision, resolved_with=2, rationale="winner")
...
def test_resolve_rejects_out_of_range(backlog, open_decision):
    with pytest.raises(ValueError, match="must be 1..3"):
        entity_helpers.resolve_decision(backlog, open_decision, resolved_with=99)


def test_drop_sets_status_dropped_with_reason(backlog, open_decision):
    fm = entity_helpers.drop_decision(backlog, open_decision, reason="superseded by external decision")
...
def test_update_decision_can_change_title_options_recommendation(backlog, open_decision):
    fm = entity_helpers.update_decision(backlog, open_decision, {
        "title": "Renamed",
        "options": ["a", "b", "c", "d"],
        "recommendation": 4,
    })
...
def test_update_decision_rejects_terminal_to_open(backlog, open_decision):
    entity_helpers.resolve_decision(backlog, open_decision, resolved_with=1)
    with pytest.raises(ValueError, match="cannot reopen"):
        entity_helpers.update_decision(backlog, open_decision, {"status": "open"})
...
def test_update_rejects_shrinking_options_below_resolved_with(backlog, open_decision):
    ...
    entity_helpers.resolve_decision(backlog, open_decision, resolved_with=3)
    with pytest.raises(ValueError, match="resolved_with must be 1..2"):
        entity_helpers.update_decision(backlog, open_decision, {"options": ["a", "b"]})
...
def test_update_allows_shrinking_options_that_keep_resolved_with_in_range(backlog, open_decision):
    ...
    entity_helpers.resolve_decision(backlog, open_decision, resolved_with=2)
    fm = entity_helpers.update_decision(backlog, open_decision, {"options": ["a", "b"]})
    assert fm["options"] == ["a", "b"]
    assert fm["resolved_with"] == 2


def test_link_handover_appends_referenced_in(backlog, open_decision):
    entity_helpers.link_decision_to_handover(backlog, open_decision, "2026-05-15-foo")
