"""Lint checks for the taskmaster:issue skill scaffolding (wrapper + playbook)."""
...
PLAYBOOK_DIR = Path(__file__).resolve().parents[1] / "playbooks" / "issue"
...
def test_wrapper_points_at_playbook():
    ...
    assert "../../playbooks/issue/playbook.md" in text
...
def test_playbook_documents_all_entry_points():
    ...
    assert not missing, f"playbook.md missing entry-point slugs: {missing}"


def test_flag_from_conversation_is_gone():
    p = PLAYBOOK_DIR / "references" / "entry-point-flows.md"
...
def test_issue_bar_reference_exists():
    assert (PLAYBOOK_DIR / "references" / "issue-bar.md").exists()
    # severity-heuristics.md must be gone (renamed)
    assert not (PLAYBOOK_DIR / "references" / "severity-heuristics.md").exists()


def test_all_referenced_files_exist():
    expected_refs = [
        PLAYBOOK_DIR / "references" / "issue-bar.md",
        PLAYBOOK_DIR / "references" / "lifecycle.md",
        PLAYBOOK_DIR / "references" / "auto-extraction.md",
        PLAYBOOK_DIR / "templates" / "issue-body.md",
    ]
