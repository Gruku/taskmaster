"""Lint checks for the taskmaster:check-todos skill (wrapper + playbook)."""
...
PLAYBOOK_DIR = Path(__file__).resolve().parents[1] / "playbooks" / "check-todos"
...
def test_wrapper_points_at_playbook():
    text = (SKILL_DIR / "SKILL.md").read_text(encoding="utf-8")
    assert "../../playbooks/check-todos/playbook.md" in text
...
def test_references_exist():
    assert (PLAYBOOK_DIR / "references" / "scan-flow.md").exists()
