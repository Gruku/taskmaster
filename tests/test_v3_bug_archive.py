...
def test_archive_bug_moves_fixed_bug_to_archive_dir(tmp_path):
    from taskmaster_v3 import write_bug, update_bug, archive_bug, bug_path
    bp = _setup(tmp_path)
...
def test_archive_bug_refuses_open(tmp_path):
    from taskmaster_v3 import write_bug, archive_bug
    bp = _setup(tmp_path)
    bid, _ = write_bug(bp, title="t", discovered_by="user")
...
def test_read_bug_falls_through_to_archive(tmp_path):
    from taskmaster_v3 import write_bug, update_bug, archive_bug, read_bug
    bp = _setup(tmp_path)
    bid, _ = write_bug(bp, title="t", discovered_by="user")
    update_bug(bp, bid, status="fixed", fix_commit="abcd")
    archive_bug(bp, bid)
