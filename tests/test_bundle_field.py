...
def test_add_task_accepts_bundle(tm_epic_phase):
    backlog_server.backlog_add_task(title="A", epic="test-epic", phase="dev", bundle="asset-ux", options={"task_id": "b-1"})
...
def test_bad_slug_rejected_on_add(tm_epic_phase):
    out = backlog_server.backlog_add_task(title="A", epic="test-epic", phase="dev", bundle="Bad Slug!", options={"task_id": "b-1"})
    assert out.lower().startswith("error")


def test_update_sets_and_clears_bundle(tm_epic_phase):
    backlog_server.backlog_add_task(title="A", epic="test-epic", phase="dev", options={"task_id": "b-1"})
